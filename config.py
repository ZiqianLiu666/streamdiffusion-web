...
parser.add_argument(
    ...
    help="Base model path or HuggingFace model ID (default: stabilityai/sd-turbo for SD-Turbo)",
)
parser.add_argument(
    "--use-lcm-lora",
    dest="use_lcm_lora",
    action="store_true",
    default=False,
    help="Use LCM-LoRA for faster inference (default: False for SD-Turbo)",
)
parser.add_argument(
    "--no-lcm-lora",
    dest="use_lcm_lora",
    action="store_false",
    help="Disable LCM-LoRA",
)
parser.add_argument(
    "--lcm-lora-id",
    dest="lcm_lora_id",
    type=str,
    default=None,
    help="LCM-LoRA model ID (default: None, uses default LCM-LoRA)",
)
parser.set_defaults(taesd=USE_TAESD, use_lcm_lora=False)
